...
const target = enviroment === "production" ? "Release" : "Debug";
const nativeClrFile = `../build/${target}/edge_nativeclr.node`;
const clrFile = process.env.EDGE_USE_CORECLR || !fs.existsSync(path.resolve(__dirname, nativeClrFile)) ? "edge_coreclr.node" : "edge_nativeclr.node";
const builtEdge = path.resolve(__dirname, `../build/${target}/${clrFile}`);
let edge;
let edgeNative;
...
function determineVersion() {
    ...
    throw new Error(`The edge module has not been pre-compiled for node.js version ${process.version}. 
        ...
        Please refer to https://github.com/agracio/edge for building instructions.`);
}
...
if (process.env.EDGE_NATIVE) {
    ...
    throw new Error(`The edge native module is not available at ${builtEdge}.
        ...
        If you need to build edge.node, follow build instructions for your platform at https://github.com/agracio/edge`);
}
...
if (edgeNative.match(/edge_coreclr\.node$/i)) {
    ...
    // how to compile literal C# at https://github.com/agracio/edge-cs/blob/master/lib/edge-cs.js
...
exports.func = function(language, options) {
    ...
    if (typeof language !== "string") {
        ...
        const compilerName = "edge-" + language.toLowerCase();
        let compiler;

        try {
            compiler = require(compilerName);
        } catch (e) {
            throw new Error(`Unsupported language '${language}'. To compile script in language '${language}' an npm module '${compilerName}' must be installed.`);
        }

        try {
            options.compiler = compiler.getCompiler();
        } catch (e) {
            throw new Error(`The '${compilerName}' module required to compile the '${language}' language does not contain getCompiler() function.`);
        }

        if (typeof options.compiler !== "string") {
            throw new Error(`The '${compilerName}' module required to compile the '${language}' language did not specify correct compiler package name or assembly.`);
        }

        if (process.env.EDGE_USE_CORECLR) {
            const defaultManifest = path.join(__dirname, "bootstrap", "bin", "Release", "netcoreapp1.1", "bootstrap.deps.json");
            ...
            options.bootstrapDependencyManifest =
                compilerManifest && fs.existsSync(compilerManifest) ? compilerManifest : defaultManifest;
        }
    }

    if (!options.assemblyFile && !options.source) {
        throw new Error("Provide DLL or source file name or .NET script literal as a string parmeter, or specify an options object with assemblyFile or source string property.");
    ...
    if (typeof options.source === "function") {
        const match = options.source.toString().match(/[^]*\/\*([^]*)\*\/\s*\}$/);
        if (match) {
    ...
    if (options.assemblyFile) {
        if (!options.typeName) {
            const matched = options.assemblyFile.match(/([^\\\/]+)\.dll$/i);
            if (!matched) {
                throw new Error("Unable to determine the namespace name based on assembly file name. Specify typeName parameter as a namespace qualified CLR type name of the application class.");
            }
